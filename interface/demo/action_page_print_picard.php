...
if (count($genderkeys[$cWRar]) > 0){
	echo "<p><span class=\"bold\">Genre :</span> $genderkeys[$cWRar]</p>";
		...
		if (count(array_filter($wrTraductionForm)) > 0 && $cWRar =='Valenciennes')  {
...
if (count(array_filter($wrDef)) > 0 && $j == 0){
	echo "<p style=\"font-weight:bold;\">Traduction : </p>";
	foreach($wrDefkeys as $key => $value){
	if ($value == $cWRar){
	echo "<p>".$key."</p>";
...
foreach ($dataLinks2 as $row){
	...
	if(count($row[wrFormConcat])>0){
		echo "<p style=\"font-weight:bold;\"> Traduction Picarde :</p><p>".$row[wrFormConcat]."</p>";
	}
	echo "</div>";
}
