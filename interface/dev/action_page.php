...
<script>
	...
	if (xAmiens != null && yAmiens != null){
	var amiens=[xAmiens, yAmiens];
var map = L.map('map').setView(amiens, 7);
}
else {
	var valenciennes=[xValenciennes, yValenciennes];
var map = L.map('map').setView(valenciennes, 7);
}
    ...
    "features": [
        {
            ...
            "type": "Feature",
            "properties": {
                "popupContent":  <?php echo '"'.addslashes(str_replace(array("\n", "\r"),'',$traductions["Valenciennes"])).'"' ?>
            },
            "id": 23
        },
        {
